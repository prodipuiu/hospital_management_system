...
<html lang="en">
  ...
  <?php
			...
			$query = "SELECT p.Room,p.Paitient_id,p.Paitient_name,d.Doctors_name FROM paitient p JOIN doctor d ON (p.Room = d.Room_incharge)";
    	...
    	<table class=" ui called unstackable table">
			...
			if ($result = $conn->query($query)) {
				while ($row = $result->fetch_assoc()){
					printf("<tr>");
					printf("<td>%s</td><td>%d</td> <td>%s</td> <td>%s</td> ",$row["Room"],$row["Paitient_id"], $row["Paitient_name"], $row["Doctors_name"]);
					printf("</tr>");
				}
			}
			?>
